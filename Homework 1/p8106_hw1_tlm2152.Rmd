...
Next, we will fit a lasso model on the training data.
...
The test RMSE noted above is around 20,500 with a tuning parameter (lambda) value of 336. Below are the 37 coefficients in the model with this tuning parameter.
...
There are 26 coefficients and the intercept in the lasso model when the "1se" rule is used. This lambda value can be seen in the plot above, shown by the right-most dashed line.
